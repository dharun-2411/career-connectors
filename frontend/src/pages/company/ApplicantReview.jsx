...
export const ApplicantReview = () => {
  ...
  return (
    ...
    >
      {/* Toast Notification */}
      {statusNotification && (
        <div className="mb-6 p-4 rounded-xl bg-indigo-50 border border-indigo-200 text-indigo-900 text-sm font-semibold flex items-center gap-2 shadow-sm animate-fade-in">
      ...
      <div className="p-4 rounded-2xl bg-white border border-slate-200/90 shadow-nexus-sm mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <Filter className="w-5 h-5 text-indigo-600" />
          <span className="text-xs font-semibold text-slate-600 uppercase tracking-wider">Filter by Job Posting:</span>
          <select
            value={opportunityId || ''}
            onChange={(e) => setSearchParams(e.target.value ? { opportunityId: e.target.value } : {})}
            className="px-3 py-2 bg-white border border-slate-200 rounded-xl text-slate-900 text-xs font-semibold focus:outline-none focus:border-indigo-500"
          >
          ...
          {aiRanking?.rankedApplicants ? (
            aiRanking.rankedApplicants.map((applicant) => {
              ...
              return (
                <div
                  ...
                  className="p-6 rounded-2xl bg-white border border-slate-200/90 space-y-4 hover:border-indigo-300 transition-all shadow-nexus-sm hover:shadow-nexus"
                >
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                      {/* Rank Badge */}
                      <div className="w-12 h-12 rounded-xl bg-indigo-600 flex items-center justify-center font-extrabold text-white text-lg shadow-sm flex-shrink-0">
                        #{applicant.rank}
                      </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {(applicant.resumeUrl || applicant.studentResumeUrl) ? (
                        <button
                          ...
                          className="px-3 py-2 rounded-xl bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 text-indigo-700 flex items-center gap-1.5 text-xs font-semibold shadow-sm transition-all cursor-pointer"
                          title="View role-specific candidate resume"
                        >
                          <FileText className="w-4 h-4 text-indigo-600" /> View Resume
                        </button>
                      ) : (
                        <span className="px-2.5 py-1.5 rounded-xl bg-slate-100 border border-slate-200 text-slate-400 text-xs font-medium flex items-center gap-1">
                          <FileText className="w-3.5 h-3.5 text-slate-400" /> No Resume
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="p-4 rounded-xl bg-indigo-50/60 border border-indigo-100 space-y-2 text-xs">
                    <div className="text-slate-700 leading-relaxed font-normal">
                      <span className="text-indigo-700 font-bold">AI Assessment: </span>
                      {applicant.aiRecommendationSummary}
                    </div>

                    <div className="flex flex-wrap items-center gap-4 pt-2 border-t border-indigo-100/80">
                      {applicant.topMatchingSkills?.length > 0 && (
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span className="text-slate-500 font-semibold">Matched Skills:</span>
                          {applicant.topMatchingSkills.map((s, i) => (
                            <span key={i} className="px-2 py-0.5 rounded bg-emerald-50 text-emerald-700 border border-emerald-200 text-[11px] font-medium">
                              {s}
                            </span>
                          ))}
                        </div>
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition-all ${
                          currentStatus === 'SHORTLISTED'
                            ...
                            : 'bg-slate-100 hover:bg-slate-200 text-purple-700 border border-slate-200'
                        }`}
                      >
            ...
            applicants.map((app) => {
              ...
              return (
                <div
                  key={app.id}
                  className="p-6 rounded-2xl bg-white border border-slate-200/90 space-y-4 hover:border-indigo-300 transition-all shadow-nexus-sm"
                >
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="space-y-1.5">
                      <div className="flex items-center gap-2">
                        <h4 className="text-base font-bold text-slate-900">{app.studentName}</h4>
                      ...
                      <div className="text-xs text-slate-500">
                        Applied for: <strong className="text-slate-800">{app.opportunityTitle}</strong> • {app.studentUniversity}
                      </div>
                    </div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-slate-100">
                    <span className="text-xs text-slate-500 font-medium">Candidate Stage:</span>
                    <div className="flex items-center gap-2 flex-wrap">
                      <button
                        type="button"
                        disabled={isUpdating}
      ...
      {viewingResume && (
        ...
        >
          <div className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                {viewingResume.url.startsWith('http') && (
                  <a
                    ...
                    className="px-3 py-1.5 rounded-lg border border-slate-200 text-xs font-semibold text-slate-700 hover:text-slate-900 bg-white hover:bg-slate-100 flex items-center gap-1.5 shadow-sm"
                  >
            ...
            <div className="w-full h-[65vh] bg-slate-50 rounded-2xl border border-slate-200 overflow-hidden flex items-center justify-center p-2">
              {viewingResume.url.startsWith('data:image/') ? (
                ...
                <div className="p-8 text-center space-y-4">
                  ...
                  <div>
                    <h4 className="text-base font-bold text-slate-900 mb-1">Resume Attached</h4>
