...
export const RoadmapSearchBox = ({
  ...
  return (
    <div ref={wrapperRef} className="relative w-full max-w-3xl mx-auto">
      <form onSubmit={handleSubmit} className="relative">
        <div className="relative flex items-center">
          ...
          <input
            ...
            className="w-full pl-12 pr-28 py-3.5 bg-white border border-slate-200 rounded-2xl text-slate-900 placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50 text-sm shadow-nexus-sm transition-all"
          />
          ...
          <button
            ...
            className="absolute right-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-semibold text-xs rounded-xl flex items-center gap-1.5 transition-all shadow-sm"
          >
            {loading ? (
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-200 rounded-2xl shadow-xl z-50 overflow-hidden divide-y divide-slate-100 animate-in fade-in slide-in-from-top-2 duration-200">
          ...
          {filteredTrending.length > 0 && (
            <div className="p-3">
              <div className="text-[11px] font-bold text-slate-500 uppercase tracking-wider px-2 py-1 flex items-center justify-between">
                ...
                <span className="text-indigo-600">AI Suggested</span>
              </div>
              <div className="space-y-1 mt-1">
                {filteredTrending.map((domain) => (
                  <button
                    ...
                    className="w-full px-3 py-2 text-left rounded-xl hover:bg-slate-50 flex items-center justify-between group transition-colors"
                  >
                    ...
                    <span className="text-[11px] px-2 py-0.5 rounded-md bg-indigo-50 border border-indigo-100 text-indigo-700 font-medium group-hover:bg-indigo-100">
              ...
              <div className="flex flex-wrap gap-1.5 mt-2 px-1">
                {recentSearches.map((term, i) => (
                  <button
                    ...
                    className="px-2.5 py-1 rounded-lg bg-white border border-slate-200 hover:bg-slate-100 text-xs text-slate-700 flex items-center gap-1 transition-colors shadow-sm"
                  >
                    <span>{term}</span>
